...
def get_droplet_classes(d_32, path, d_max=3e-3, s=0.32, plot=False):
    ...
    if plot==True:
        ...
        ax.set_title('Volume-based probability density distribution \n $d_{32}$=' + str(d_32*1e6) +'$\mu m$'
                    ...
                    + ', \n number of classes=' + str(constants.N_D))
        # save plot
...
def initialize_boundary_conditions(epsilon_in, d_32, d_max, path, plot=False):
    '''
    ...
    N_in_total, n_in_rel, d_bins = get_totalNumber_water_inlet(epsilon_in,d_32,d_max=d_max, V_mix=V_mix, path=path) # total number of droplets at inlet
    # relative dropsize distribution at inlet
    get_droplet_classes(d_32, d_max=d_max, path=path, plot=plot)
