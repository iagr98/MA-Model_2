...
def run_mainscript(input):
    # name_of_simulation = 'DN200_L1.8m_Vin_4.0_phase_fraction_0.33_SauterIn_0.5'
    # do_plot = True
    Q_in, phasefraction_in, Sauterdiameter_in, Q_w_out, q_o_out, r_v, y0, t_end, name_of_simulation, do_plot = input
    try:
        os.mkdir('results')
    except:
        pass
    ...
    path = 'results' + str("\\") + name_of_simulation
    ...
    Q_IN = Q_in # total flow rate inlet m3/s
    EPSILON_IN = phasefraction_in # phase fraction of organic phase -
    # boundary conditions / inlet conditions
    d_32 = Sauterdiameter_in # diameter of a droplet m at separator inlet
    ...
    df['Coalescence parameter'] = r_v*np.ones(N_t)
